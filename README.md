...
南瓜价格预测数据分析项目 README
项目概述
本项目旨在通过对南瓜相关数据进行分析，构建预测模型来预测南瓜的平均价格。它涵盖了从数据加载、预处理、模型构建、评估到模型改进和解释的完整数据分析流程。
代码功能详解
首先，代码导入了一系列必要的库，包括用于数据处理的 pandas、模型训练与评估的 sklearn 相关模块、数值计算的 numpy 以及数据可视化的 matplotlib.pyplot。接着，从指定路径 C:\Users\Lenovo\.ssh\xiaoxueqi - 666\US - pumpkins.csv 加载南瓜数据文件，查看数据的基本信息，如列名、数据类型、非空值数量等。根据数据的行数和列数判断是短表数据（行数少于 100 且列数少于 20）还是长表数据，分别查看全量数据信息或前几行数据信息，从而了解数据内容。

在数据预处理阶段，计算南瓜的平均价格作为预测目标，选择可能影响平均价格的特征变量（Type、Package、Variety、Item Size、Color、Repack）并提取特征和目标变量的数据。对特征变量中的缺失值进行填充，将缺失值标记为 unknown，然后对分类变量进行独热编码，将其转换为适合模型处理的数值形式。

随后进行模型构建与评估。先将数据集划分为训练集和测试集，其中测试集占比 20%。创建线性回归模型并在训练集上进行训练，使用训练好的模型在测试集上进行预测，计算模型的均方误差、均方根误差和决定系数等评估指标，以衡量模型的性能并输出评估结果。

为了改进模型，创建了随机森林回归模型，设置树的数量为 100。在训练集上训练该模型后，使用它在测试集上进行预测，同样计算均方误差、均方根误差和决定系数等评估指标并输出结果，以便与线性回归模型进行对比。

最后进行模型解释，获取随机森林回归模型的特征重要性，将特征重要性和特征名称对应起来，对特征重要性进行降序排序并取前 10 个特征。设置图片清晰度和中文字体后，绘制特征重要性柱状图展示前 10 个重要特征，同时输出这些特征的重要性数值，从而深入理解各特征对南瓜平均价格预测的影响。
