...
plt.title('各特征的f_regression得分（得分越高，与目标变量相关性越强）')
...
# 可视化XGBoost特征重要性（新增可视化）
plt.figure(figsize=(8, 6))
xgb.plot_importance(model, height=0.8, importance_type='gain', title='XGBoost特征重要性（基于增益）')
plt.tight_layout()
plt.show()
...
cv_results = cross_val_score(model, X_new_df, y, cv=kfold, scoring='neg_mean_squared_error')
rmse_scores = np.sqrt(-cv_results)

print('\n===== 5折交叉验证结果 =====')
print(f'交叉验证RMSE均值：{rmse_scores.mean():.4f} ± {rmse_scores.std():.4f}')
print('各折RMSE：', [f'{score:.4f}' for score in rmse_scores])